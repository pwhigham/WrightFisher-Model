...
Updated: 25/2/2020
This model is the single locus but writes out the structure of the
population (at present just counts) for each generation.  This can
be used to match Kimura's data for M<deltax>, V<deltax>
...
#define FALSE 0
#define TRUE 1
#define SMALLRANDOM 0.0000000001
...
void printNetwork()
{
	...
	{
		for(int j=0;j<N;++j)
			printf(network[i][j]>0?"1":"0"); //network[i][j]);
...
double rand01()
{
	...
	return((res==(double)0.0)?(double)SMALLRANDOM:res); 
}
/*
...
void calc_cumsum(int locn)
{
	...
	for(register int i=0;i<N;++i)
	{
		...
		cumsum[i] = total;  /* Just set to whatever was before or updated total */
	}
...
int main(int argc, char *argv[])
{
	...
	if (argc<7)
	{
		...
		printf("N = %d\n", N);
		printf("\nSingle Locus Model - s gives selection added value (i.e. 1+s)\n");
		printf("\nRuns single time step for <numruns>, recording #mutG0 and #mutG1\n");
		return(1);
	}
	sscanf (argv[1],"%f",&p);
	sscanf (argv[2],"%f",&s);
	...
	{			
		...
		popstep(p);
		printf("%d,%.2f,%.2f,%d,%d\n",
				N,s,p,fixedResult[0],fixedResult[1]);
	}
}
